...
var promises = [d3.json("countries-110m.json"), d3.json(`${BASE_URL}/wsi`)];
...
function ready([world, response]) {
  ...
  if (years.includes(year) == false) year = years[0];
  ...
  // Create the Timeline
  timeline = new Timeline("#titlebar", {
    margin: {
      top: 0,
      bottom: 30,
      left: 0,
...
function updateData(yr) {
  ...
  if (yr < 2020 || (deltaGdp == 0 && deltaPop == 0)) map.data = data[yr];
  // Otherwise show the predicted data
  else map.data = predict[yr];

  // Show or hide the sliders
  if (yr > 2015) $("#editor").addClass("open");
  else {
